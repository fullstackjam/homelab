...
The website is generated using [Material for MkDocs](https://squidfunk.github.io/mkdocs-material) and can be viewed at [homelab.fullstackjam.dev](https://homelab.fullstackjam.dev).
...
<!-- - Find the TXT record for `homelab.fullstackjam.dev` and switch the `ownerID` between `homelab` and `horus` -->
